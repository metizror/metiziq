interface FileReadyUserTemplateData {
  customerName: string;
  recordCount: number;
  downloadUrl: string;
  baseUrl?: string;
...
export function getFileReadyUserTemplate(data: FileReadyUserTemplateData): string {
  const { customerName, recordCount, downloadUrl, baseUrl = "" } = data;
  ...
  return `
    ...
    <html lang="en">
      <head>
        ...
        <title>Your AMFAccess File is Ready</title>
      </head>
                ...
                <tr>
                  <td style="padding: 40px 40px 30px 40px; text-align: left;">
                    <h1 style="margin: 0 0 20px 0; font-size: 24px; font-weight: 600; color: #111827; line-height: 1.3;">
                      Your AMFAccess File is Ready
                    </h1>
                    <p style="margin: 0 0 20px 0; font-size: 16px; line-height: 1.6; color: #374151;">
                      Hi ${customerName},
                    </p>
                    <p style="margin: 0 0 20px 0; font-size: 16px; line-height: 1.6; color: #374151;">
                      Your file containing ${recordCount} records is now ready.
                    </p>
